...
from numpy import array, mean, std, log as numpy_log, cov, ones, inf,maximum,argmax
...
def cal_proditrate_withlock(pricelist, signallist, takeprofitpoint,stoplosspoint,plotornot=False, timeline=None):
    ...
    record = False
    record_filename = str(int(time())) + ".csv"
    lock = False
    lock_trend = None
    while i < len(signallist) - 2:
        ...
        if signallist[i] != 0 :
            if record == True :
                with open(record_filename, 'a') as myfile:
                    csvwriter = writer(myfile)
                    csvwriter.writerow(['-' for i in range(168)])
                    csvwriter.writerow(["打开订单", timeline[i], "仓位方向", signallist[i]])

            print("-" * 168)

            if lock==True:
                if lock_trend!=signallist[i]:
                    print("取消Lock")
                    trend = signallist[i] if lock_trend!=signallist[i] else None
                    cost_price = pricelist[i] if lock_trend!=signallist[i] else None
                    print("打开订单", timeline[i], "仓位方向", signallist[i])
                    lock=False
                    lock_trend=None
                    #sleep(0.1)
                else:
            ...
            if i + 1 < len(signallist) - 1:
                while signallist[i + 1] != 0 and signallist[i + 1] == trend:
                    if i < len(signallist) - 2:
                        changes=False
                        if (trend * float(pricelist[i] - cost_price) / cost_price - trade_cost * (
                                1 + pricelist[i] / cost_price)) >= abs(takeprofitpoint):
                            print("Lock One!")
                            lock = True
                            lock_trend = trend
                            each_order_yield.append(
                                trend * float(pricelist[i] - cost_price) / cost_price - trade_cost * (
                                        1 + pricelist[i] / cost_price))
                            profitrate += (trend * float(pricelist[i] - cost_price) / cost_price - trade_cost * (
                                    1 + pricelist[i] / cost_price))
                            culmulative_profitrate.append(
                                profitrate + (trend * float(pricelist[i] - cost_price) / cost_price - trade_cost * (
                                        1 + pricelist[i] / cost_price)))
                            print("该次订单利润率", trend * float(pricelist[i] - cost_price) / cost_price - trade_cost * (
                                    1 + pricelist[i] / cost_price))
                            print("关闭订单", timeline[i])
                            trend = None
                            cost_price = None
                            changes = True

                        elif (trend * float(pricelist[i] - cost_price) / cost_price - trade_cost * (
                                1 + pricelist[i] / cost_price)) < -abs(stoplosspoint):
                            print("Lock One!")
                            lock = True
                            lock_trend = trend
                            each_order_yield.append(
                            ...
                            trend = -trend
                            cost_price = pricelist[i]
                            print("打开fanzhuan订单", timeline[i], "仓位方向", trend)

                        else:
                            pass
                        if changes==False:
                            culmulative_profitrate.append(profitrate + (trend * float(
                            pricelist[i] - cost_price) / cost_price - trade_cost) if trend!=None else profitrate)
                        i += 1
                    else:
                        profitrate += (
                                trend * float(pricelist[len(pricelist) - 1] - cost_price) / cost_price - trade_cost * (
                                1 + pricelist[len(pricelist) - 1] / cost_price))
                        ...
                        print("该次订单利润率",
                              trend * float(pricelist[len(pricelist) - 1] - cost_price) / cost_price - trade_cost * (
                                      1 + pricelist[len(pricelist) - 1] / cost_price))
                        print("关闭订单", timeline[i])
                        right += 1 if trend * float(
                            pricelist[len(pricelist) - 1] - cost_price) / cost_price - trade_cost * (
                                              1 + pricelist[len(pricelist) - 1] / cost_price) > 0 else 0
                        if record == True:
                            with open(record_filename, 'a') as myfile:
                                csvwriter = writer(myfile)
                                csvwriter.writerow(["该次订单利润率",
                                                    trend * float(pricelist[len(
                                                        pricelist) - 1] - cost_price) / cost_price - trade_cost * (
                                                            1 + pricelist[len(pricelist) - 1] / cost_price)])
                                csvwriter.writerow(["关闭订单", timeline[i]])
                    ...
                    if trend != None:
                        ...
                        if record == True:
                            with open(record_filename, 'a') as myfile:
                                ...
                                csvwriter.writerow(["该次订单利润率",
                                                    trend * float(
                                                        pricelist[i] - cost_price) / cost_price - trade_cost * (
                                                            1 + pricelist[i] / cost_price)])
                                csvwriter.writerow(["关闭订单", timeline[i]])
                        print("该次订单利润率", trend * float(pricelist[i] - cost_price) / cost_price - trade_cost * (
                                    1 + pricelist[i] / cost_price))
                        print("关闭订单", timeline[i])
                        right += 1 if trend * float(pricelist[i] - cost_price) / cost_price - trade_cost * (
                                    1 + pricelist[i] / cost_price) > 0 else 0
                        times += 1
    ...
    fig.canvas.set_window_title("1:实时净值;2:累积净值;3:BTC")
    show()

    sigma = std(each_tragy_diffratio)
    lowest_cul_profitrate = argmax(
        (maximum.accumulate(culmulative_profitrate) - culmulative_profitrate) / maximum.accumulate(
            culmulative_profitrate))
    highest_in_min = argmax(culmulative_profitrate[:lowest_cul_profitrate])
    max_retreat = (culmulative_profitrate[lowest_cul_profitrate] - culmulative_profitrate[highest_in_min])
    sharp_ratio = (profitrate - 1 - norisk_yield * len(pricelist) / (365 * 12 * 24)) / (
            sigma * sqrt((365 * 12 * 24))) if sigma > 0 else 0
    calmar_ratio = (profitrate - 1 - norisk_yield * len(pricelist) / (365 * 5 * 24)) / abs(
        max_retreat) if max_retreat < 0 else 0
    CAPM_resultr = OLS(each_tragy_diffratio, add_constant(array(each_long_diffratio).T)).fit()
    max_order_loss = min(each_order_yield)
    G_calmar_ratio = (profitrate - 1 - norisk_yield * len(pricelist) / (365 * 5 * 24)) / abs(
        max_order_loss) if max_order_loss < 0 else 1e+234
    print(CAPM_resultr.summary())
    alpha = CAPM_resultr.params[0] * 365 * 24 * 12
    beta = CAPM_resultr.params[1]

    print("交易费率", trade_cost, "单边")
    print("----------Summary---------")
    print("最大的10个盈利订单",sorted(each_order_yield)[-1:-10:-1])
    print("最大的10个亏损订单", sorted(each_order_yield)[:10])
    print("净利润率:", profitrate - 1, "* 100%")
    print("Sigma:", sigma, "* 100%")
    print("每笔订单平均利润率", mean(each_order_yield))
    print("每笔订单利润率标准差", std(each_order_yield))
    print("最大持续亏损:", max_retreat)
    print("最大回撤:", max_retreat / culmulative_profitrate[highest_in_min])
    print("夏普比率:", sharp_ratio)
    print("Calmar比率:", calmar_ratio)
    print("净利润率减无风险收益率除以最大单笔订单亏损", G_calmar_ratio)
    print("最大单笔订单亏损", max_order_loss)
    print("年化 Alpha:", alpha, "Alpha t 检验", CAPM_resultr.params[0] / CAPM_resultr.HC0_se[0])
    print("Beta:", beta, "Beta t 检验", CAPM_resultr.params[1] / (CAPM_resultr.HC0_se[1]))
    print("CAPM模型 R squared", CAPM_resultr.rsquared_adj)
    print("盈利单数 num:", right, "总共订单数", times)
    print("胜率", float(right) / float(times), "* 100%")
def cal_profitrate(pricelist, signallist, plotornot=False, timeline=None, **judge_model):
            ...
            if i + 1 < len(signallist) - 1:
                ...
                while signallist[i + 1] != 0 and signallist[i + 1] == trend:
                    if i < len(signallist) - 2:
                        ...
                        print("该次订单利润率",
                              trend * float(pricelist[len(pricelist) - 1] - cost_price) / cost_price - trade_cost * (
                                          1 + pricelist[len(pricelist) - 1] / cost_price))
                        print("关闭订单", timeline[i])
                        right+=1 if trend * float(pricelist[len(pricelist) - 1] - cost_price) / cost_price - trade_cost * (
                                        1 + pricelist[len(pricelist) - 1] / cost_price)>0 else 0
                        if record==True:
                            with open(record_filename, 'a') as myfile:
                                csvwriter = writer(myfile)
                    ...
                    if trend != None:
                        ...
                        culmulative_profitrate.append(
                            profitrate + (trend * float(pricelist[i] - cost_price) / cost_price - trade_cost*(1+pricelist[i] / cost_price)))
                        each_order_yield.append((trend * float(pricelist[i] - cost_price) / cost_price - trade_cost*(1+pricelist[i] / cost_price)))
                        if record==True:
                            with open(record_filename, 'a') as myfile:
                                csvwriter = writer(myfile)
                                csvwriter.writerow(["该次订单利润率",
                              trend * float(pricelist[i] - cost_price) / cost_price - trade_cost * (
                                          1 + pricelist[i] / cost_price)])
                                csvwriter.writerow(["关闭订单", timeline[i]])
                        print("this profit rate", trend * float(pricelist[i] - cost_price) / cost_price - trade_cost*(1+pricelist[i] / cost_price))
                        print("close order at", timeline[i])
                        right+=1 if trend * float(pricelist[i] - cost_price) / cost_price - trade_cost*(1+pricelist[i] / cost_price)>0 else 0
                        times += 1
                    else:
                        print("pass")
            else:
    ...
    sigma = std(each_tragy_diffratio)
    lowest_cul_profitrate= argmax((maximum.accumulate(culmulative_profitrate) - culmulative_profitrate) /maximum.accumulate(culmulative_profitrate))
    highest_in_min= argmax(culmulative_profitrate[:lowest_cul_profitrate])
    max_retreat =(culmulative_profitrate[lowest_cul_profitrate] -culmulative_profitrate[highest_in_min])
    sharp_ratio = (profitrate - 1 - norisk_yield * len(pricelist) / (365 * 12 * 24)) / (
                sigma * sqrt((365 * 12 * 24))) if sigma > 0 else 0
    ...
    CAPM_resultr = OLS(each_tragy_diffratio, add_constant(array(each_long_diffratio).T)).fit()
    minorder_nums = 5 * 12
    max_order_loss = min(each_order_yield) if len(each_order_yield) > minorder_nums else -1
    G_calmar_ratio = (profitrate - 1 - norisk_yield * len(pricelist) / (365 * 5 * 24)) / abs(
        max_order_loss) if max_order_loss < 0 else 1e+234
    print(CAPM_resultr.summary())
    alpha = CAPM_resultr.params[0] * 365 * 24 * 12
    ...
    print("每笔订单平均利润率",mean(each_order_yield))
    print("每笔订单利润率标准差",std(each_order_yield))
    print("最大持续亏损占回撤前最高净值比例:", max_retreat)
    print("最大回撤:", max_retreat/culmulative_profitrate[highest_in_min])
    print("夏普比率:", sharp_ratio)
    print("Calmar比率:", calmar_ratio)
    print("净利润率减无风险收益率除以最大单笔订单亏损", G_calmar_ratio)
    ...
    print("胜率",float(right)/float(times),"* 100%")
    return -(1 * profitrate -
             sqrt(len(pricelist)) * sigma +
             10 * max_order_loss+
             100*float(right)/float(times))
...
def weight_indicators(pricelist):
    ...
    while repeat <= sample_size:
        ...
        sigma = std(each_tragy_diffratio)
        highest_in_min = argmax(
            (maximum.accumulate(culmulative_profitrate) - culmulative_profitrate) / maximum.accumulate(
                culmulative_profitrate))
        lowest_cul_profitrate = argmax(culmulative_profitrate[:highest_in_min])
        max_retreat = -(culmulative_profitrate[lowest_cul_profitrate] - culmulative_profitrate[highest_in_min])
        CAPM_resultr = OLS(each_tragy_diffratio, add_constant(array(each_long_diffratio).T)).fit()
        ...
        minorder_nums = int(len(pricelist) / (average_holding_period * 25 * 12))
        max_order_loss = min(each_order_yield) if len(each_order_yield) > minorder_nums else 0
        calmar_ratio = (profitrate - 1 - norisk_yield * len(pricelist) / (365 * 5 * 24)) / abs(
            max_retreat) if max_retreat < 0 else 0
        G_calmar_ratio = (profitrate - 1 - norisk_yield * len(pricelist) / (365 * 5 * 24)) / abs(
            max_order_loss) if max_order_loss < 0 else 0
        repeat+=1
        ...
        G_calmar_list.append(G_calmar_ratio)
        alpha_list.append(alpha)
        beta_list.append(beta)
        CAPM_Rsquare.append(CAPM_resultr.rsquared)
        print(repeat / sample_size)
    ...
    print("max_retreat_list mean", mean(max_retreat_list), "std", std(max_retreat_list), "weight",
          1 / std(max_retreat_list))
    print("sharp_ratio_list mean", mean(sharp_ratio_list), "std", std(sharp_ratio_list), "weight",
          1 / std(sharp_ratio_list))
    print("calmar_ratio_list mean", mean(calmar_ratio_list), "std", std(calmar_ratio_list), "weight",
          1 / std(calmar_ratio_list))
    print("G-calmar_ratio_list mean", mean(G_calmar_list), "std", std(G_calmar_list), "weight",
          1 / std(G_calmar_list))
    ...
    print("G-calmar_ratio_list mean", mean(CAPM_Rsquare), "std", std(CAPM_Rsquare), "weight",
          1 / std(CAPM_Rsquare))
        ...
        if signallist[i] != 0:
            ...
            culmulative_profitrate.append(
                profitrate + (trend * float(
                    pricelist[i] - cost_price) / cost_price - trade_cost) if trend != None else profitrate)
            list_profitrate.append(
                trend * float(pricelist[i] - cost_price) / cost_price - trade_cost if trend != None else 0)
            i += 1
    sigma = std(each_tragy_diffratio)
...
def cal_accuracy(pricelist, signallist):
    ...
    if total == 0:
        if cost_price != None:
            total += 1
            accuracy += 1 if (trend * (
                        pricelist[len(pricelist) - 1] - cost_price) / cost_price - trade_cost) >= 0 else 0
...
def Kline_image(high, low, open, close):
    Image = lambda High_price, Low_price, Open_price, Close_price: (Close_price - Open_price) / (
                High_price - Low_price) if (High_price - Low_price) != 0 else float(1)
...
def Yield_gap(price, Param_T):
    def cal_Yield_list(input_price):
        ...
        Yield_list = (array(input_price[:len(input_price) - 1]) - array(lag_price)) / array(
            input_price[:len(input_price) - 1])
...
def weighted_continuous_signals(close, gap_interval=10, total_num=30):
    ...
    for index_interval in range(gap_interval, gap_interval + total_num * gap_interval, gap_interval):
        got_list = tag_signal_continuous(close, gap_interval + index_interval * gap_interval,
                                         trade_cost=trade_cost * 2.2)
...
class ML_GLM_test:
    def __init__(self, sample_file='bitmex_5min.csv', param_k=6, param_S=100, param_T=100):
        self.time_list = []
        self.timestamplist = []
        self.my_close_list = []
        self.my_max_list = []
    ...
    def Weighted_build(self):
        ...
        self.W_signals = max_min(got_weight_continuous_signals, max=max(got_weight_continuous_signals),
                                 min=min(got_weight_continuous_signals))
        ...
        for i in range(0, len(self.my_close_list) - self.param_S - 1, self.param_S):
            ...
            if (max(temp_list) - min(temp_list)) / min(temp_list) > trade_cost * mutlti:
                if index_max < index_min:
                    ...
                    if (temp_list[self.param_S - 1] - temp_list[index_min]) / temp_list[
                        index_min] > trade_cost * mutlti:
    ...
    def generate_factor(self):
        ...
        self.linepart = [
            (self.my_close_list[i] - self.my_open_list[i]) / (self.my_max_list[i] - self.my_min_list[i]) if (
                                                                                                                        self.my_max_list[
                                                                                                                            i] -
                                                                                                                        self.my_min_list[
                                                                                                                            i]) != 0 else None
            for i in range(0, len(self.my_close_list))]
        self.elastic = [(prev_close_list[i] - lag_close[i]) * lag_volume[i] / (
        ...
        for l in range(0, len(self.linepart)):
            if self.linepart[l] == None:
                self.linepart[l] = max_linepart if (self.my_max_list[l] - self.my_min_list[
                    l]) > 0 else -1 * max_linepart
    ...
    def fit_bymethod(self, method, *params):
        ...
        if method == 'ols':
            ...
            print(method, 'superparam', params, '||', reg.score(self.data_to_fit_x, self.data_to_fit_y),
                  '||scores in testset', reg.score(self.data_to_test_x, self.data_to_test_y))
        elif method == 'rls':
            try:
                assert params != None
            ...
            clf = neural_network.MLPClassifier(solver='adam', hidden_layer_sizes=(params[0], params[1]),
                                               random_state=params[2] if len(params) > 2 else None)
            clf.fit(self.data_to_fit_x, self.data_to_fit_y)
            print(method, 'superparam', params, '||', clf.score(self.data_to_fit_x, self.data_to_fit_y),
                  '||scores in testset', clf.score(self.data_to_test_x, self.data_to_test_y))
        elif method == 'adaboost':
            clf = ensemble.AdaBoostClassifier(algorithm='SAMME', n_estimators=5,
                                              base_estimator=linear_model.LogisticRegression() if params[
                                                                                                      0] == 'logistic' else None,
                                              random_state=0)
            clf.fit(self.data_to_fit_x, self.data_to_fit_y)
            print(method, 'superparam', params, '||', clf.score(self.data_to_fit_x, self.data_to_fit_y),
                  '||scores in testset', clf.score(self.data_to_test_x, self.data_to_test_y))
            ...
            while stop == False:
                ...
                if self.tostr(group_index) in had_list:
                    ...
                    print("model group:", dict_clf[model.index(now_group[0])], dict_clf[model.index(now_group[1])],
                          dict_clf[model.index(now_group[2])])
                    clf = ensemble.VotingClassifier(estimators=[(dict_clf[model.index(now_group[0])], now_group[0]),
                                                                (dict_clf[model.index(now_group[1])], now_group[1]),
                                                                (dict_clf[model.index(now_group[2])], now_group[2])],
                                                    voting='hard')
                    clf.fit(self.data_to_fit_x, self.data_to_fit_y)
                    print(method, 'superparam', params, '||', clf.score(self.data_to_fit_x, self.data_to_fit_y),
                          '||scores in testset', clf.score(self.data_to_test_x, self.data_to_test_y))
            ...
            clf = ensemble.StackingClassifer(estimators=[('dt', clf1), ('knn', clf2), ('svc', clf3)],
                                             final_estimstor=linear_model.LogisticRegression())
            clf.fit(self.data_to_fit_x, self.data_to_fit_y)
            print(method, 'superparam', params, '||', clf.score(self.data_to_fit_x, self.data_to_fit_y),
                  '||scores in testset', clf.score(self.data_to_test_x, self.data_to_test_y))
        else:
    ...
    def transfer_test(self):
        ...
        print("proifit", cal_profitrate(test_price, logisitic_signal, plotornot=True, timeline=self.otime_list),
              'accurary', cal_accuracy(test_price, logisitic_signal))
    ...
    def neuralnetwork_modeltest(self):
        ...
        x_train = array(self.factor_list[:int(len(self.factor_list) * 0.7)]).reshape(
            (int(len(self.factor_list) * 0.7), 1, (self.param_k - 1) * 4))
        x_test = array(self.factor_list[int(len(self.factor_list) * 0.7):]).reshape(
            (len(self.factor_list) - int(len(self.factor_list) * 0.7), 1, (self.param_k - 1) * 4))

        x_train2_complete = array(self.factor_list_2_complete[:int(len(self.factor_list_2_complete) * 0.7)]).reshape(
            (int(len(self.factor_list_2_complete) * 0.7), 1, (self.param_k - 1) * 4))
        ...
        for i in range(len(model2_test)):
            ...
            if ks == 1:
                ...
                goes_signals.append(0)
        print(cal_profitrate(self.my_close_list[self.param_k:], signallist=goes_signals,
                             timeline=self.time_list[self.param_k:]))
    ...
    def goto_test(self):
        ...
        print(cal_profitrate(self.my_close_list[self.param_k:], signal_list, plotornot=True,
                             timeline=self.time_list[self.param_k:]))
        print(cal_accuracy(self.my_close_list[self.param_k:], signal_list))
    def BOLLRSI_stradegy(self, DEVparam_up, DEVparam_down, linear_cons,linear_beta,linear_beta_forma,linear_beta_forBOLL,Sinparam,Cosparam,param_T, type='price'):
        ...
        test_list = numpy_log(array(diffratio) + ones(len(diffratio))) if type == 'logyield' else array(
            self.my_close_list)
        BOLL_UPPER, BOLL_MA, BOLL_DOWNER = BBANDS(test_list, timeperiod=param_T, nbdevup=Upper_openposition_params,
                                                  nbdevdn=Downer_openposition_params
                                                  )
        rsi = RSI(array(self.my_close_list), param_T) / 100
        trend = lambda x: 1 if x > 0 else -1
        tanh = lambda x: (exp(x)-exp(-x))/(exp(x)+exp(-x)) if abs(x)<10 else trend(x)

        def judge_signal(x, upper, downer, ma, rsi_i):
            BOLL_situation=lambda X,MA,UP,DOWN:(X-MA)/(UP-DOWN) if UP-DOWN>0 else 1e+234
            situation_ma = (array(x) - array(ma)) / array(x)
            situation_boll = array([BOLL_situation(x[i],ma[i],upper[i],downer[i]) for i in range(len(x))])
            if x > ma + upper * Sinparam:
                return -1 * closest_3(tanh(
                    linear_cons + array(rsi_i).dot(linear_beta) + array(situation_boll).dot(linear_beta_forBOLL) +array(situation_ma).dot(linear_beta_forma )))
            elif x < ma + downer * Cosparam:
                return closest_3(tanh(
                    linear_cons + linear_beta * rsi_i + linear_beta_forBOLL * situation_boll + linear_beta_forma * situation_ma))
            else:
                return 0
        rsi_lag=[rsi[i-self.param_t:i] for i in range(len(rsi))]
        upper_lag=[BOLL_UPPER[i-self.param_t:i] for i in range(len(BOLL_UPPER))]
        downer_lag=[BOLL_DOWNER[i-self.param_t:i] for i in range(len(BOLL_DOWNER))]
        ma_lag=[BOLL_MA[i-self.param_t:i] for i in range(len(BOLL_MA))]
        Signals=lambda z:uniform(-1,1)
        BOLL_signals = list(
            map(Signals, test_list[param_T:]))
        BACKTESTs=BACKTEST(price=self.my_close_list[param_T:], signal=BOLL_signals,time=self.time_list[param_T:],timestamp=self.timestamplist[param_T:],period=1/(12*24),Concretesignal=False,Maxordernumbers=3)
        print("sdf")

    def BOLL_Stradegy(self, DEVparam_up, DEVparam_down, Cosparam, Sinparam, param_T, type='price',**other):
        Upper_openposition_params = DEVparam_up  # 0.6744897501960817
        Downer_openposition_params = DEVparam_down  # 0.6744897501960817
        temp_price = self.my_close_list[:-1]
        temp_diffratio = list((array(self.my_close_list[1:]) - array(temp_price)) / array(temp_price))
        ...
        def judge_signal(x, upper, downer, ma):
            if x > ma and ma + upper * Sinparam >= x >= ma + upper * Cosparam:
                return 1
            elif x > ma + upper * Sinparam:
                return -1
            elif x > ma and x <= ma + upper * Cosparam:
                return 0
            elif x < ma and ma + downer * Sinparam <= x <= ma + downer * Cosparam:
                return -1
            elif x < ma + downer * Sinparam:
                return 1
            elif x < ma and x >= ma + downer * Cosparam:
                return 0
            else:
                return 0
        Signals=lambda :choice([-1,0,1])
        BOLL_signals = list(map(Signals, test_list[param_T:], BOLL_UPPER, BOLL_DOWNER, BOLL_MA))
        print("score:",cal_profitrate(self.my_close_list[param_T:], BOLL_signals, plotornot=True,
                             timeline=self.time_list[param_T:])if len(other)==0 else cal_proditrate_withlock(self.my_close_list[param_T:], BOLL_signals, plotornot=True,
                             timeline=self.time_list[param_T:],takeprofitpoint=other['takeprofit'],stoplosspoint=other['stoploss']))
        return cal_profitfunc_noprint(self.my_close_list[param_T+self.param_t:], BOLL_signals)


    def opt_BOLL_Stradegy(self):
        bounds = Bounds(lb=[0,1,0], ub=[inf,inf,1])
        chouyang=lambda (normalvariate(1,18)
        initial_x0=list(map,[None]))
        K_list = []
        value_list = []
        params_list = []
        for k in range(10, 1000):
            print("Now index:", k)
            min_func = lambda x,sin,cos: self.BOLL_Stradegy(DEVparam=x, Sinparam=sin,Cosparam=cos,param_T=k)
            opt_result = minimize(fun=min_func,x0=array(initial_x0), bounds=bounds, options={'maxiter': 120})
            K_list.append(k)
            print("This min val:", opt_result.fun, "Best param:", opt_result.x, "gap:", opt_result.x - array(initial_x0))
    ...
    def iter_reg(self):
        ...
        funcs = lambda x: opt_calprofitrate(self.my_close_list[self.param_k:int(len(self.my_close_list) * 0.2)], x,
                                            self.factor_list[:int(len(self.my_close_list) * 0.2)])
    ...
    def final_regression(self):
        ...
        for i in range(self.param_k + int(self.param_S / 2), len(self.tagged_line)):
            ...
            try:
                ...
                doba = 1 if int(
                    logistic_1.predict([allset_for_X_do[i - self.param_k - int(self.param_S / 2)]])[0]) == 0 else 0
                ...
                now_signal_2.append(
                    logistic_2.predict([allset_for_X[i - self.param_k - int(self.param_S / 2)]])[0] * int(
                        logistic_1.predict([allset_for_X_do[i - self.param_k - int(self.param_S / 2)]])[0]))

            except:
                print("sdfds")

        print("profit rate",
              cal_profitrate(self.my_close_list[self.param_k + int(self.param_S / 2):], now_signal_2, plotornot=True,
                             timeline=self.time_list))
        print("accuracy rate", cal_accuracy(self.my_close_list[self.param_k + int(self.param_S / 2):], now_signal_2))
        print("profit rate",
              cal_profitrate(self.my_close_list[self.param_k:], now_signal, plotornot=True, timeline=self.time_list,
        ...
        print("-----------------------")
        print("profit rate",
              cal_profitrate(self.my_close_list[self.param_k:], now_signal_4, plotornot=True, timeline=self.time_list,
                             judge_model=[prob_donotdo, prob_buysell]))
        print("accuracy rate", cal_accuracy(self.my_close_list[self.param_k:], now_signal_4))
        print("dgdf")

trade_cost = float(0.00025)
a = ML_GLM_test(param_k=121, param_S=10, param_T=13)
a.opt_BOLL_Stradegy()
