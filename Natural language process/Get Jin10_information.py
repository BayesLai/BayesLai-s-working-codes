...
def get_news(unix_time):
    ...
    for i in range(len(loads_result['newest_news'])-1,0,-1):
        ...
        if last_news_time==None:
            ...
            loads_result['newest_news'][i]['data']['content'] if 'content' in loads_result['newest_news'][i]['data'].keys() else loads_result['newest_news'][i]['data']['country']+loads_result['newest_news'][i]['data']['name']+loads_result['newest_news'][i]['data']['actual']+','+loads_result['newest_news'][i]['data']['previous'],
            loads_result['newest_news'][i]['important'],
            loads_result['newest_news'][i]['channel'],
            loads_result['newest_news'][i]['remark']
        ])
        else:
            if news_time+60*60*8<=last_news_time:
                ...
                record_to_csv([
                    ...
                    loads_result['newest_news'][i]['type'],
                    # 如果是文字内容，则直接读取content内容，否则使用字符串操作符讲数据重新组合成文本语句
                    loads_result['newest_news'][i]['data']['content'] if 'content' in loads_result['newest_news'][i]['data'].keys() else loads_result['newest_news'][i]['data']['country']
                         + loads_result['newest_news'][i]['data']['name'] + str(loads_result['newest_news'][i]['data']['actual']) + ',' + str(loads_result['newest_news'][i]['data']['previous']),
                    loads_result['newest_news'][i]['important'],
                    loads_result['newest_news'][i]['channel'],
                    loads_result['newest_news'][i]['remark']
                ])
