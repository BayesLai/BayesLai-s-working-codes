...
def get_news(unix_time):
    ...
    req = Request(url="https://www.jin10.com/flash_newest.js?" + str(int(unix_time)))
    ...
    counts = 0
    for i in loads_result['newest_news']:
        news_time = mktime(utc_2_pk(i['time']).timetuple())
        ...
        if last_news_time==None:
            ...
            i['data']['content'] if 'content' in i['data'].keys() else i['data']['country']+i['data']['name']+i['data']['name']+i['data']['actual']+','+i['data']['previous'],
            i['important'],
            i['channel'],
            i['remark']
        ])
            ...
            if news_time<=last_news_time:
                ...
                record_to_csv([
                    ...
                    i['type'],
                    # 如果是文字内容，则直接读取content内容，否则使用字符串操作符讲数据重新组合成文本语句
                    i['data']['content'] if 'content' in i['data'].keys() else i['data']['country'] + i['data'][
                        'name'] + i['data']['name'] + i['data']['actual'] + ',' + i['data']['previous'],
                    i['important'],
                    i['channel'],
                    i['remark']
                ])
...
while 1<10:
    try:
        ...
        #控制频率,防止被目标网站ban掉
        print("Updated One")
        sleep(abs(normalvariate(10*20,5)))
        #每步往前读取30分钟的内容
        now_time-=60*30
    except:
        print(exc_info())
