...
def top_words_data_frame(model: LatentDirichletAllocation,
    ...
    for topic in model.components_:
        ...
        top_words.insert(0,'Topic '+str(int(len(rows)+1)))
        rows.append(top_words)
    columns = [f'Word {i+1}' for i in range(n_top_words)]
    columns.insert(0,'which topic')
...
# 输出主题词的文件路径
top_words_csv_path = 'Top listed words(updating).csv'
# 输出各文档所属主题的文件路径
predict_topic_csv_path = 'Probability of each topic(updating).csv'
